...
后续精修主参考文档：`docs/项目深度分析与精修方案.md`。所有 agent 执行任务前必须先读该文档，再读 `PLAN.md` 和 `docs/技术方案.md`。
...
详细流程见 `.codex/agents/`。本项目不再维护 `.claude/agents` 和 `CLAUDE.md`。
...
1. 文档和 agent 收敛：以 `docs/项目深度分析与精修方案.md` 为后续精修主依据，只保留 Codex agent。
2. 工程基线：依赖声明、测试入口、类型规范、基础 CI 预备。
3. 项目文件格式：`project.facecat/` 分离资产、分析运行、人工标注和导出。
4. 视频质量门控：模糊、光照、脸部占比、遮挡、极端头姿。
5. 候选事件检测：基于 AU/blendshape 曲线生成 onset/apex/offset。
6. 标注闭环：自动候选和人工确认分离存储，导出包含来源、质量和模型版本。
7. 语音/语言轨接入离线批处理和 UI。
...
- 禁止节注释横幅，用方法名表达意图。
- 单个方法尽量不超过 50 行。
- 项目依赖入口为 `pyproject.toml`；新增依赖时同步更新可选 extras。
...
| 2026-05-23 | 新增项目深度分析与精修方案，合并 agent 入口，只保留 Codex agent |
